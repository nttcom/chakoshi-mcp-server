...
Claude Desktop などからchakoshiの Guardrails Apply API を利用して、テキストのガードレールチェック（モデレーション、PIIフィルタ、キーワードフィルタ等）を実行できます。
...
### ガードレールの作成
⚠️ Guardrails Apply API を利用するためには、ポリシー設定をあらかじめ完了してガードレールIDを発行している必要があります。
